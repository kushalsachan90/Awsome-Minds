...
const parseInstanceIdFromArn = (arn) => {
  ...
  if (
    ...
    throw new Error(
      `Unable to parse EC2 instance ID from ARN: ${arn}`
    );
  }
...
export const handleEc2Fix = async (event) => {
  ...
  if (event.resourceType !== "EC2") {
    throw new Error(
      `Unsupported resource type: ${event.resourceType}`
    );
  }
  ...
  switch (action) {
    ...
    case "START_INSTANCE":
      return await startInstance(event, parameters);
...
const startInstance = async (event, parameters) => {
  const { resourceArn, incidentId } = event;

  const instanceId =
    parseInstanceIdFromArn(resourceArn);

  const instance =
    await describeInstance(instanceId);
  ...
  if (state === "pending") {
    return {
      ...
      actionTaken: "START_INSTANCE",
      instanceId,
      status: "START_ALREADY_REQUESTED",
      nextStep: "WAIT_AND_CHECK_RUNNING",
    };
  }

  if (state !== "stopped") {
    throw new Error(
      `Cannot start ${instanceId}: current state is "${state}"`
    );
  }
...
const resizeInstance = async (event, parameters) => {
  ...
  if (state === "running") {
    console.log(
      "Stopping EC2 instance before resize:",
      {
        instanceId,
        currentInstanceType,
        targetInstanceType,
...
const restartWithRollbackAmi = async (
  event,
  parameters
) => {
  const { resourceArn, incidentId } = event;
  ...
  if (
    !targetAmiId ||
    !/^ami-[a-f0-9]{8,17}$/.test(targetAmiId)
  ) {
    throw new Error(
      `Invalid or missing targetAmiId: ${targetAmiId}`
    );
  }
  ...
  if (!["running", "stopped"].includes(state)) {
    throw new Error(
      `Cannot rollback ${instanceId}: ` +
      `current state is "${state}"`
    );
  }

  const existingReplacement =
    await findExistingReplacement(
      incidentId,
      instanceId
    );
  ...
  if (existingReplacement) {
    return {
      ...
      nextStep:
        "WAIT_AND_HEALTH_CHECK_REPLACEMENT",
    };
  }

  const securityGroupIds =
    (original.SecurityGroups || [])
      .map((sg) => sg.GroupId)
      .filter(Boolean);

  console.log(
    "Launching EC2 replacement from rollback AMI:",
    {
      originalInstanceId: instanceId,
      targetAmiId,
    }
  ...
  const runResponse =
    await ec2Client.send(
      new RunInstancesCommand({
        ...
        IamInstanceProfile:
          original.IamInstanceProfile?.Arn
            ? {
                Arn:
                  original.IamInstanceProfile.Arn,
              }
  ...
  const newInstanceId =
    runResponse.Instances?.[0]?.InstanceId;

  if (!newInstanceId) {
    throw new Error(
      "RunInstances did not return a new instance ID"
    );
  }
...
const findExistingReplacement = async (
  incidentId,
  originalInstanceId
) => {
  const response =
    await ec2Client.send(
      new DescribeInstancesCommand({
        Filters: [
          {
            Name:
              "tag:SelfHealing:IncidentId",
            Values: [incidentId],
          },
          {
            Name:
              "tag:SelfHealing:ReplacesInstance",
            Values: [originalInstanceId],
          },
          {
            Name:
              "tag:SelfHealing:Action",
            Values: [
              "RESTART_WITH_ROLLBACK_AMI",
            ],
          },
